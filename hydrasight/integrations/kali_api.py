...
class KaliAPI:
    ...
    def run(self, command: str, timeout: int = 300) -> dict:
        ...
        try:
            ...
            return {
                "output": "", "error": "API request timeout",
                "returncode": -1, "success": False, "timed_out": True,
            }
        except requests.ConnectionError:
            self.log.error("API connection refused")
            return {
                "output": "", "error": "kali-server-mcp not reachable",
    ...
    def local_ip(self, target: str) -> str:
        res   = self.run(
            f"ip route get {target} | grep -oP 'src \\K\\S+'", timeout=10
        )
    ...
    def check_target(self, target: str) -> dict:
        res = self.run(
            f"ping -c 2 -W 2 {target} 2>&1 | tail -3", timeout=15
        )
