...
class Parser:
    ...
    KIWI_USER_RE = re.compile(r"Username\s*:\s*(?:[^\s\\]+\\)?(\S+)", re.IGNORECASE)
    KIWI_NTLM_RE = re.compile(r"NTLM\s*:\s*([a-fA-F0-9]{32})", re.IGNORECASE)
    DIR_RE = re.compile(r"(?:^|\s)(/[^\s(]*)\s+\(Status:\s*(\d+)\)", re.MULTILINE)
    ...
    OS_RE = re.compile(r"OS\s*(?:Name)?\s*:\s*(.+?)$", re.IGNORECASE | re.MULTILINE)
    ANONYMOUS_FTP = re.compile(r"Anonymous (?:FTP )?login allowed", re.IGNORECASE)
    SMB_SHARE_RE = re.compile(r"^\s*([A-Z0-9_$]+)\s+(?:Disk|IPC)", re.MULTILINE)
    ...
    def hashes(cls, out: str) -> list[dict]:
        ...
        users = [(m.group(1).strip(), m.end()) for m in cls.KIWI_USER_RE.finditer(out)]
        ntlms = [(m.group(1).strip().lower(), m.start()) for m in cls.KIWI_NTLM_RE.finditer(out)]
    ...
    @classmethod
    def session_id(cls, out: str) -> int | None:
    ...
    @classmethod
    def os_info(cls, out: str) -> str | None:
    ...
    def validate(cls, tool: str, out: str, elapsed: float) -> list[str]:
        ...
        if tool in min_times and elapsed < min_times[tool]:
            warnings.append(f"completed in {elapsed:.1f}s — possible error or empty result")
        if not out.strip():
