...
from hydrasight.config.defaults import P, SEV, TOOL_LABELS, PHASE_DEFS
...
def phase_header(
    phase_id: str, phase_label: str, color: str, idx: int, total: int
) -> None:
...
def task_line(tool: str) -> None:
    console.print(
        f"\n  [{P.MUTED}]exec[/]    "
        f"[{P.PRIMARY}]{TOOL_LABELS.get(tool, tool)}[/]"
    )


def result_line(
    tool: str, elapsed: float, chars: int, warnings: list[str]
) -> None:
                ...
                if val is None:
                    ...
                    for item in val:
                        if isinstance(item, dict):
                            ...
                            parts.append(
                                f"[{sev.upper()}] {desc}" if sev else desc
                            )
                        else:
...
def make_table(*cols: tuple) -> Table:
    t = Table(
        ...
        header_style=P.MUTED, border_style=P.DIM, padding=(0, 2),
    )
