...
def test_basic_command_rendering():
    spec = CommandSpec(
        ...
        args=[
            ...
            CommandPart("80,443", quote=True),
            CommandPart("10.10.10.10", quote=True),
        ],
    )
...
def test_shell_metacharacter_rejection():
    ...
    spec = CommandSpec(
        executable="ping",
        args=[
            CommandPart("-c", quote=False),
            CommandPart("1", quote=False),
            CommandPart("127.0.0.1; id", quote=False),
        ],
    )
    rendered = CommandBuilder.build(spec)
    assert not rendered.is_safe
    assert (
        "Unquoted argument contains shell metacharacters: 127.0.0.1; id"
        in rendered.validation_errors
    )


def test_shell_metacharacter_quoted_is_safe():
    # If quoted, it's treated as a single argument safely by shlex.quote
    spec = CommandSpec(executable="echo", args=[CommandPart("hello; id", quote=True)])
    rendered = CommandBuilder.build(spec)
    assert rendered.is_safe
